...
import { Users, Settings2, Gauge, MapPin, CheckCircle2, ChevronRight, Star, Car, Heart } from 'lucide-react'
...
export default function ListCard({ car, onNavigate }: { car: CarType; onNavigate: (id: string) => void }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row overflow-hidden min-h-50 sm:h-52">
            <div className="relative w-full sm:w-48 md:w-56 shrink-0 h-52 sm:h-full cursor-pointer" onClick={() => onNavigate(car._id!)}>
                {imgSrc && !imgError ? (
                    <img src={imgSrc} alt={car.title} onError={() => setImgError(true)} className="w-full h-full object-cover" />
                ) : (
                    <div className="w-full h-full min-h-30 flex items-center justify-center bg-gray-50">
                ...
                >
                    <Heart size={15} fill={has(car._id!) ? '#cc0000' : 'none'} className={has(car._id!) ? 'text-red-500' : 'text-gray-600'} />
                </button>
            </div>
            ...
            <div className="flex-1 p-4 flex flex-col sm:flex-row gap-3 justify-between min-w-0">
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 sm:gap-2 border-t sm:border-t-0 pt-3 sm:pt-0 shrink-0 sm:min-w-30">
                    {car.rating !== undefined && (
                        <div className="flex items-center gap-2">
                            ...
                            <div className="bg-[#003580] text-white text-xs font-bold px-2 py-1 rounded-lg rounded-tr-none min-w-8 text-center">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-xl font-black text-gray-900">{format(car.pricePerDay)}</p>
                    </div>
                    <button
