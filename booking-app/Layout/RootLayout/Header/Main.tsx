...
export default function Header() {
    ...
    const [guests, setGuests] = useState<{ adults: number; children: number; rooms: number }>({ adults: 2, children: 0, rooms: 1 });
    const [entireHome, setEntireHome] = useState<boolean>(false);
            ...
            <div className="bg-[#003b94] px-3 sm:px-6 pb-6 sm:pb-10">
                <div className="max-w-6xl mx-auto sm:translate-y-25">
                    ...
                    <div className="flex flex-col sm:flex-row sm:flex-wrap gap-1 bg-[#febb02] p-1 rounded-lg">
                                ...
                                <div className="absolute top-[calc(100%+4px)] left-0 z-50 bg-white rounded-xl shadow-2xl min-w-60 overflow-hidden">
                                    {SUGGESTIONS
                                        ...
                                        .map((s: string) => (
                                            <div
                                                key={s}
                                                onClick={() => { setLocation(s); setShowLocation(false); }}
                            ...
                            {showDate && (
                                <div className="absolute top-[calc(100%+4px)] left-0 z-50 bg-white rounded-xl shadow-2xl min-w-150">
                                    ...
                                    <div className="flex border-b border-gray-100">
                                        {["calendar", "flexible"].map((tab: string) => (
                                            <button
                                                key={tab}
                                                onClick={() => setCalendarTab(tab)}
                                                className={`flex-1 py-3 text-sm font-medium transition-colors ${calendarTab === tab
                                        ...
                                        <div key={key} className="flex justify-between items-center py-3 border-b border-gray-100 last:border-0">
                                            ...
                                            <div className="flex items-center gap-3">
                                                <button
                                                    ...
                                                    className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-[#003b94] hover:border-[#003b94] transition-colors"
                                                >
                                                    <Minus size={13} />
                                                </button>
                                                <span className="w-5 text-center text-sm font-semibold text-gray-800">{guests[key]}</span>
                                                <button
                                                    onClick={() => adjust(key, 1)}
                                                    className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-[#003b94] hover:border-[#003b94] transition-colors"
                                                >
                        ...
                        <button 
                            onClick={() => {
                                const isCarPage = window.location.pathname.includes('carrender');
                                const target = isCarPage ? '/carresults' : '/filter';
                                const params = new URLSearchParams();
                                if (location) params.set('city', location);
                                
                                if (isCarPage) {
                                    if (checkIn) params.set('pickUp', checkIn.toISOString().split('T')[0]);
                                    if (checkOut) params.set('dropOff', checkOut.toISOString().split('T')[0]);
                                } else {
